...
pub(crate) const PANEL_MIN_SIZE: Pixels = px(0.);
...
impl ResizableState {
    ...
    pub(crate) fn update_panel_size(
        ...
        // Update the size if it's still at the default PANEL_MIN_SIZE,
        // OR if the panel has an explicit size set that differs from cached
        let should_update = self.sizes[panel_ix].as_f32() == PANEL_MIN_SIZE.as_f32()
            || (self.panels[panel_ix].size.is_some()
                && (size - self.sizes[panel_ix]).abs() > px(1.));
